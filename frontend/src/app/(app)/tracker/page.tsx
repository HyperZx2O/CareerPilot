...
export default function TrackerPage() {
  ...
  async function loadAll(uid: string) {
    try {
      const [appsList, goalsList, todosList] = await Promise.all([
        getApplications(uid),
        getGoals(uid),
        getTodos(uid),
      ]);
      ...
      setGoals(goalsList);
      setTodos(todosList);
      if (goalsList.length > 0 && !selectedGoalId) {
  ...
  async function handleDeleteGoal(id: string) {
    const prevGoals = useAppStore.getState().goals;
    const prevSelected = selectedGoalId;
    setGoals(prevGoals.filter((g) => g.id !== id));
    if (selectedGoalId === id) setSelectedGoalId(prevGoals[0]?.id ?? "");
    try {
      await deleteGoal(id);
    } catch {
      setGoals(prevGoals);
      setSelectedGoalId(prevSelected);
  ...
  async function handleToggleTodo(todo: Todo) {
    ...
    const prevTodos = useAppStore.getState().todos;
    setTodos(prevTodos.map((t) => (t.id === todo.id ? { ...t, done: newDone } : t)));
    try {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    const prevApps = useAppStore.getState().applications;
    setApplications([newApp, ...prevApps]);
    try {
